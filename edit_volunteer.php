...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        ...
        $stmt = $db->prepare("SELECT email FROM volunteers WHERE email = ? AND volunteer_id != ?");
        $stmt->bind_param("si", $email, $volunteer_id);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows > 0) {
    ...
    if (empty($errors)) {
        $stmt = $db->prepare("UPDATE volunteers SET 
            first_name = ?, 
            last_name = ?, 
            email = ?, 
            phone = ?, 
        ...
        if ($stmt->execute()) {
            ...
            $errors['database'] = 'Error updating volunteer: ' . $db->error;
        }
        $stmt->close();
    }
} else {
...
<head>
  ...
  <title>VolunteerHub - Edit Volunteer</title>
  ...
  <div class="flex">
    ...
    <main class="flex-1 p-6">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Edit Volunteer</h2>
        <a href="view_volunteer.php?id=<?php echo $volunteer_id; ?>" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 transition-colors">
          Cancel
        </a>
      </div>
